...
const form = {
	...
	*/
			...
			for (let i = 0, length = rule.length; i < length; i++) {
				...
				switch (ruleItem) {
					case "required":
						...
						break;
					case "isMobile":
						isError = formData[key]?!form._isMobile(formData[key]):false;
						break;
					case "isEmail":
						isError = formData[key]?!form._isEmail(formData[key]):false;
						break;
					case "isCarNo":
						isError = formData[key]?!form._isCarNo(formData[key]):false;
						break;
					case "isVin":
						isError = formData[key]?!form._isVin(formData[key]):false;
						break;
					case "isIdCard":
						isError = formData[key]?!form._isIdCard(formData[key]):false;
						break;
					case "isAmount":
						isError = formData[key]?!form._isAmount(formData[key]):false;
						break;
					case "isNum":
						isError = formData[key]?!form._isNum(formData[key]):false;
						break;
					case "isChinese":
						isError = formData[key]?!form._isChinese(formData[key]):false;
						break;
					case "isEnglish":
						isError = formData[key]?!form._isEnglish(formData[key]):false;
						break;
					case "isEnAndNo":
						isError = formData[key]?!form._isEnAndNo(formData[key]):false;
						break;
					case "isEnOrNo":
						isError = formData[key]?!form._isEnOrNo(formData[key]):false;
						break;
					case "isSpecial":
						isError = formData[key]?form._isSpecial(formData[key]):false;
						break;
					case "isEmoji":
						isError = formData[key]?form._isEmoji(formData[key]):false;
						break;
					case "isDate":
						isError = formData[key]?!form._isDate(formData[key]):false;
						break;
					case "isUrl":
						isError = formData[key]?!form._isUrl(formData[key]):false;
						break;
					case "isSame":
						isError = formData[key]?!form._isSame(formData[key], formData[value]):false;
						break;
					case "range":
						let range = null;
						try {
						...
						break;
					case "minLength":
						isError = formData[key]?!form._minLength(formData[key], value):false
						break;
					case "maxLength":
						isError = formData[key]?!form._maxLength(formData[key], value):false
						break;
					default:
						break;
				}
	...
	_isVin: function(value) {
		return /^[A-Ha-hJ-Nj-nPpR-Zr-z0-9]{17}$/.test(value);
	},
